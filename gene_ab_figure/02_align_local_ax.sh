#!/bin/bash
# Step 2 (local, base-level): align each long-read sample to the 276-genome
# community reference with minimap2 -ax map-ont (matching the original cluster
# 02_build_and_align.sh), streaming SAM into count_stream_sam.py. Base-level
# alignment correctly assigns reads among homologous denitrification-gene copies.
set -uo pipefail
...
mkdir -p $DATA/logs
rm -f $DATA/.alignment_complete
echo "[$(date +%T)] START -ax alignment of 5 long-read samples (ref=276 MAGs)"

for sample in CAN_1 CAN_2 CAN_3 CAN_4 CAN_5; do
    reads=$READS/${sample}_nanopore.fastq.gz
    out=$DATA/counts_${sample}.json
    log=$DATA/logs/minimap2_ax_${sample}.log
    ...
    $MM2 -ax map-ont -t $THREADS --secondary=no -K 2g "$REF" "$reads" 2> "$log" \
        | $PY $WORK/count_stream_sam.py --bed "$BED" --sample "$sample" --out "$out"
    rc=${PIPESTATUS[0]}
